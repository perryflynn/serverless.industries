  ...
  class TargetBlank
    ...
    class << self
      ...
      def process_anchor_tags(html)
        ...
        anchors = content.css("#{custom_css_selector} a[href]")
      ...
      # Private: Determines if the processing should be restricted to a css selector
      #
      # Returns selector or empty string
      def custom_css_selector
        config = @target_blank_config
        case config
        when nil, NilClass
          ''
        else
          return config.fetch("selector", '')
        end
      end
